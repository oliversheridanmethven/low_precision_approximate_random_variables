...
\keywords{approximations, random variables, inverse cumulative distribution functions, random number generation, finite precision, half precision, floating point, rounding error, multilevel Monte Carlo, the Euler-Maruyama scheme, the Milstein scheme, Kahan compensated summation, and high performance computing.}
...
\Description{The piecewise linear approximation closely resembles the Gaussian distribution's inverse cumulative distribution function with a high fidelity. The resultant probability density function is a sequence of piecewise constant values whose shape somewhat resembles the exact probability density function. There are a few points of discontinuity which are bounded in size, and it has finite support.}
\end{figure}
...
\Description{Rounding to nearest even for various possible input values. For single values landing between two representable numbers, the value is simple rounded up or down to the nearest number. For two values landing within a triplet of representable numbers, the two only round differently when one of the values lies exactly half way between two values, engaging the round to nearest even tie break rule.}
\end{figure}
...
\Description{The variance of the two way multilevel difference with and without Kahan compensated summation. For the two way differences for very low precisions the variance increases primarily due to rounding error at the predicted rate. For very high precisions, the variance appears a constant for various discretisations, as the error from approximating the Gaussian distribution dominates. When using Kahan compensated summation, all the variances appear a constant. There are gaps between low and medium precision levels. However, for intermediate and higher precisions, there is no gap in the variance when using Kahan compensated summation, again showing the error is primarily due to approximating the Gaussian distribution.}
\end{figure}
...
\Description{The variance for the various multilevel terms. The double precision variance is moderately high but decreases at the usual rate with strong convergence order a half. Similar decay is observed for the two way differences in low precisions, except where after a few levels the half precision two way difference starts to increase due to rounding error. The same effect similarly happens for when using Kahan compensated summation, albeit at a finer discretisation. The variance of the four way difference in single precision is much lower than the the two way difference, and decreases at a similar rate, until at very fine discretisations when rounding error sets in. For four way differences in half precision, the onset of rounding error is immediate, and without Kahan compensated summation the rounding error quickly increases up to the two way variance after just a few levels. Using Kahan compensated summation for the four way difference in half precision gives a constant variance which extends for several levels, only coinciding with the two way difference at a more intermediate discretisation level.}
\end{figure}
...
\Description{The speed ups from using the low precision nested multilevel Monte Carlo estimators. The single precision offers a speed up of approximately seven for most discretisations, although this diminishes for very fine discretisations. For half precision without Kahan compensation, at the very coarsest two levels the speed up is approximately a factor of 10--12, although that is rapidly diminishing as the levels become finer. The half precision scheme using Kahan compensated summation is similar, except with a slightly less impressively initial speed up of a factor of approximately 9, but the speed ups diminish at a much slower rate, giving a brief intermediate region where it is the fastest of the three choices, although not by a particularly large or competitive margin.}
\end{figure}
