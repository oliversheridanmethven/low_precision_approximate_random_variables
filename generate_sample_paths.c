...
int main(void)
{
    unsigned int n_paths_per_level = 3;
    ...
    for (unsigned int level = 1; level <= max_level; level++)
    {
        ...
        double uniforms_64[n_increments];
        float32 uniforms_32[n_increments];
        half16 uniforms_16[n_increments];

//        Generate the uniforms
        for(unsigned int i = 0; i < n_increments; i++)
        {
            uniforms_16[i] = ((half16) (((double) (rand() + 1)) / ((double) (RAND_MAX + 2)) / 2.0));  // We divide by 2 so all numbers are in 0.5 so we don't need the complementary inverse CDF for uniforms rounded up towards 1 by typecasting.
            uniforms_64[i] = uniforms_32[i] = uniforms_16[i];
            weiner_increments_exact[i] = gsl_cdf_ugaussian_Pinv(uniforms_64[i]);
        }

//        Generate the Gaussian increments.
        piecewise_linear_approximation_single(n_increments, uniforms_32, weiner_increments_approx_32);
        piecewise_linear_approximation_half(n_increments, uniforms_16, weiner_increments_approx_16);
...
//            We correct for the sign of the Weiner increments as the uniforms were in (0, 0.5) and not (0, 1) by randomly negating.
            bool flip_sign = (((double) (rand() + 1)) / ((double) (RAND_MAX + 2))) > 0.5 ? true: false;
            if (flip_sign)
            {
        ...
        for (unsigned int simulation; simulation < n_paths_per_level; simulation++)
        {
...
//            printf("dt,x_fine_exact_64,x_coarse_exact_64,x_fine_approx_32,x_coarse_approx_32,x_fine_approx_16,x_coarse_approx_16,x_fine_approx_16_kahan,x_coarse_approx_16_kahan\n");
            printf("%f,%f,%f,%f,%f,%f,%f,%f,%f\n",dt,x_fine_exact_64,x_coarse_exact_64,x_fine_approx_32,x_coarse_approx_32,x_fine_approx_16,x_coarse_approx_16,x_fine_approx_16_kahan,x_coarse_approx_16_kahan);
        }
